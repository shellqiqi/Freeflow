...
/* 析构FFR，对分配的共享内存池shm_pool与内存表shm_map做释放 */
...
/* 用于FastPath Channel中的Control Message，初始state为idle */
...
/* 启动FFR，建立与FFL的交流，对应论文5 */
void FreeFlowRouter::start()
{
    LOG_INFO("FreeFlowRouter Starting... ");

    /* 创建FastPath进程，对应论文5.2，将ffr自身传入ctrl_args */
    if (!disable_rdma)
    {
        /* 该进程与rkey有关 */
        start_udp_server();
...
/* FastPath的Loop处理函数，作为线程占用一个CPU核心，处理QP、CQ、SRQ操作 */
...
/* Unix socket的Loop处理函数，处理RDMA function call */
...
/**
 * UDP用来接收FFL发来的IBV_REG_MR_MAPPING_REQ
 * 从而建立rkey到share mem的映射？
 */
